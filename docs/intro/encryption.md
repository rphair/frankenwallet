...
... which must include a more detailed question, even if your confidence in the Frankenwallet's security is absolute:  

{: .highlight }
"What prevents the files and data that I encrypt there from being broken into as soon as they're stored and transmitted through the outside world?"
...
1. [7-Zip](https://www.7-zip.org) which maintains [`7z` format](https://www.7-zip.org/7z.html) archives (e.g. for key files)
1. [LUKS](https://en.wikipedia.org/wiki/Linux_Unified_Key_Setup) (for the encrypted disk partition itself)

The reassurance posted by the foremost expert in this field at the time — also a major influencer of realistic perceptions of cryptography through the decades ever since — is that the number of bit rearrangements necessary to even count through all the possible decryption keys would still be beyond native human conception: as it still is nearly 30 years after this statement was published:  

> These numbers [**ed. summary**: indicating stellar if not galactic energy requirements for all these bit rearrangements] have nothing to do with the technology of the devices; they are the maximums that thermodynamics will allow. And they strongly imply that brute-force attacks against 256-bit keys will be infeasible until computers are built from something other than matter and occupy something other than space.  
...
In other words, the cracking problem of brute-forcing the keys is one of either cosmic or geological scale: depending on whether you consider space or time.
...
Linux itself is just as susceptible to malware downloads as any other OS like Windows would be, but our instructions (with any exceptions justified by their own security references) will include only those that come from the Linux software repositories — including Libreoffice, `7z`, and `srm`.
...
Libreoffice, [ever since version 3.5](https://help.libreoffice.org) (in [year 2012](https://www.libreoffice.org/about-us/libreoffice-timeline)), has encrypted all document data with 256-bit AES for all ODF files: i.e. when saving in its native Writer (Document), Calc (Spreadsheet), and related formats.

As this announcement indicates and source code view confirms, all document data is encrypted *inside the document structure itself* according to a key that's only created from whatever password you enter when you "Save as ...".

### Could a reclaimable version of the password somehow be stored in that unencrypted part of the file or archive? {#back-door-metadata}

This would be a "back door" for the file itself: rather than the much greater challenge of putting a [back door in the encryption algorithm](#back-door-in-cipher)... more likely leaving a "key under the doormat" for the password or decryption key to be left around in the file metadata.

The reassurance on this point is the same as for the [chain of software delivery](#repositories): any code to do this would have to be entirely new and so be reviewed by other code contributors, release managers, and repository maintainers before distributing it even to initial audiences.

But an even greater reassurance would be what it would do *to the affected files themselves*.  It is not only human beings reading encrypted LibreOffice documents and 7z archives... they are also routinely read and catalogued into document systems that parse these files.  Anyone working on such a system would see this extraneous data being put into all archive or document files and would naturally question and report it: making even such an unlikely tampering very short-lived before that part of the code would be cut out.
...
Sort of "bugs" in the program that leak a password into the operating environment somehow, that leaves the possibility of deliberate "hacking" by reading or poking at the system or running program's private memory space to see if these tiny opportunities could be exploited.

In practice, these types of opportunities are extremely difficult even under the ideal circumstance where a malicious intruder has already "hacked" a desktop or server and therefore has the opportunity for statistical attacks that might eventually find a region of system memory where a password is stored in cleartext.

If you think this seems practically almost impossible, you are absolutely right.  This is made more statistically impossible by the Frankenwallet's installation being "private" and there not known to be a target for such a difficult and long-term attack that could be only conducted by a specialised expert: who has somehow compromised your device during the limited amount of time you use it, through its limited (or no) Internet usage.  

### Could bugs appear that leave passwords exposed to due incompetence of the developers? {#inadvertent-bugs} 

We have to assume this is theoretically possible: and that is where the open source model comes in.  As you can see from the LibreOffice "release note" above, the code for full 256-bit AES encryption from a user-entered password has been in place since 2012 (thirteen years, at the time of this writing) with no reason to modify it... *and* all kinds of alarms that would go off in software repositories if it were.

The reassurance of `7z` — which has [similar longevity](https://www.7-zip.org/history.txt) for the essential components of its source code & encryption mechanisms — would be greater since passwords are conveyed through either system-protected terminal input (when using the CLI) or the native Linux password & keychain libraries to handle password securely (when encrypting files from the file manager).
...
1.  Any alteration of the AES algorithm creating an encrypted data stream can only be decrypted by the same algorithm: therefore any program writing data with anything other than *bona fide* AES wouldn't be able to read it with AES.
...
1.  The best way to avoid ensuring the unbreakable encryption of a document with absolutely priceless contents is to handle the file "Linux distro on a USB stick" — almost as conceived for the Frankenwallet (minus the notion of persistence, but including the encryption of all persistent files).
2.  The encryption of AES itself — plus the assurances that there are no artefacts in the file to provide any clue at all in its decryption — have long meant that any LibreOffice file itself without its saved password is "lost" even to its creator.
...
{: .important }
Note that these suggestions apply also to *all* Linux operations (not Windows, which is designed to constantly leak personal data of all kinds) **even outside the Frankenwallet** so can also provide privacy and security advantages in normal host system operation.  

In fact this is the basis for keeping LibreOffice files encrypted whenever possible with the Frankenwallet [lower security password](/prepare/password-low) — so even resources copied and created in the Frankenwallet can still be relatively secure even when accessing & modifying them on the host system.
