...
Since scripts can be changed so easily — and have many lines of code that operators couldn't comb all the time for intrusions (though you can [verify their checksums](https://github.com/gitmachtl/scripts/tree/master/cardano/mainnet#how-to-installcopy-the-scripts)) — we have to avoid the possibility that a compromised host system could add back doors, key extraction, and other malicious code into the SPO Script files.
...
### Install script package from your host folder
...
The home subdirectory `~/bin` is a good choice since it already exists in your `$PATH`: if using a different location, ensure you add it to `$PATH` by editing `~/.profile` accordingly.

Remember you can run `df .` (to show the filesystem where you currently are) any time you need to check you're on a Frankenwallet disk instead of your host computer's disk.

➤ Extract the contents and copy all the files in `scripts-master/cardano/mainnet` into that `bin` directory; e.g.:

```  
cp scripts-master/cardano/mainnet/* ~/bin  
```

### Put SPO scripts in [offline mode](https://github.com/gitmachtl/scripts/tree/master/cardano/mainnet#online-mode-vs-light-mode-vs-offline-mode) and finish configuration
...
➤ In the section after that, ensure the variable `cardano-cli` (and any others, if necessary) points to the place where it's actually installed (if not in the same `~/bin` directory as suggested).

## Import pool data from any existing pool

Your installation is finished here if you intend only to use the Frankenwallet + scripts to create a *new* pool.

Otherwise: to administer your existing pool, follow the SPO Scripts documentation [Import your existing data in Offline-Mode](https://github.com/gitmachtl/scripts/tree/master/cardano/mainnet#import-your-existing-data-in-offline-mode) — to create a subdirectory containing all of your pool data — with some added precautions to avoid installing sensitive data where it won't be shielded from the host computer:
...
If there's any doubt, any time you need to ensure you're not in the host folder or _any_ part of the filesystems on your host machine:
...
Once your pool data is arranged in a single place, if interested in using the Frankenwallet to generate a securely encrypted backup, you can how follow the procedure to LINK(back up your SPO Scripts pool directory).
