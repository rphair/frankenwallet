...
## What to do when `cardano-node` isn't running on your host machine
...
Although some operators would feel comfortable installing the SPO Scripts in *Online-Mode* on one of these machines — to host their information-gathering and execution — for most operators it will be more convenient to run the scripts in *Light-Mode* on the host machine itself.
...
However, the most important requirement is to authenticate that the Frankenwallet can host the same workflow as the 2 separate "offline" and "online" machines in the workflow mandated by the SPO Scripts.

Therefore we will use the simpler example of an elementary transaction: so operators will be able to verify the security and practicality of their arrangement before migrating their stake pool(LINK back to previous section "Import") into it.
...
### Example: Sending funds securely with `01_sendLovelaces.sh`
...
➤ Obtain necessary information from online machine

The file `01_workOffline.sh` will be built in your "online" SPO Scripts directory as per the example: saving the balances from the wallets used in your transaction.

Once the `offlineTransfer.json` file is complete, copy it to the Frankenwallet [host folder.](/usage/host-folder)  (If your host machine and the machine with your online SPO scripts are different, you'll also have to transfer it between machines.)

➤ Generate transaction in the Frankenwallet
...
{: .important }
This should always involve moving a file from one directory to another... since the Frankenwallet's copy of the SPO scripts cannot be installed *anywhere* on the host machine — *including* the host folder — to prevent tampering with the scripts.

When you're done using the `01_sendLovelaces.sh` script to update the `offlineTransfer.json` file, move it back from the SPO Scripts directory into the host folder again: for execution in the "online" environment.

➤ Submit transaction through online machine
...
## Backing up the SPO Scripts pool directory

Operators interested in backing up this material outside the Frankenwallet will generally follow the procedure in [Backups from Frankenwallet to host machine](/usage/backups), but with one important difference:
...
- This has been intended to allow users and operators to access less confidential files more freely on the host system... for example, stake and payment addresses which can be used and transmitted openly.
