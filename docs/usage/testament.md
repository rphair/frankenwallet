...
This is a special case of "resource sharing" for a single draft of confidential data: not expected for regular operational use, but requiring an equally strict approach to security (plus an additional need for privacy) and therefore following a very similar procedure.
...
- **Infallibility**: without the usual opportunities to clarify errors or correct omissions, you must be sure enough correct detail has been left in place for a likely naive recipient to reconstruct your portfolio: since there will be no way of answering questions "at the other end" after your incapacitation or demise.
...
Unless you have implicit trust in a person you might the contents to in advance — and unless you would trust them to cooperate with you in asset sharing *prior to* your incapacitation or demise — an encrypted LibreOffice document must be sufficient to reconstruct all your blockchain & custodial assets **as if you were deliberately giving them everything here and now**... even though they should only see this testament after your death or incapacitation is confirmed.  

{: .important-title }
> splitting the estate
>
> Use common sense in following these instructions: e.g. if you wish to strictly split your crypto estate into two parts, for two completely different sets of inheritors, you would go through *all* instructions here twice: preparing two separate documents, and situating each document and password with two different pairs of custodians.  
...
➤ Your own most trusted associates in the crypto community: any of whom is likely to want to honour your legacy and ensure your loved ones are provided for.  
...
➤ Responsible I.T. companies who could designate a trusted agent to help your executor in accessing blockchain resources under contract; perhaps after signing a non-disclosure agreement or posting a bond against theft.
...
Here you should take no chances that even a technically competent helper will immediately know *any* completely safe way (in an Internet full of scams and fakes) to convert your crypto keys and passphrases into usable reconstructed accounts.  For instance, imagine someone unfamiliar with the Cardano blockchain searching online for "cardano wallet" and finding a well-promoted fake that would wipe out your estate immediately.
...
2.  Install Lace wallet from web site **lace.io** where you will find a button "Add to Desktop" which will set up the wallet software as a browser extension.
3.  If that button is missing or not working, install the Lace extension directly from its official page ([on Firefox](https://addons.mozilla.org/en-GB/firefox/addon/lace-wallet/)) or ([on Chrome](https://chromewebstore.google.com/detail/lace/gafhhkghbfjjkeiendhlofajokpaflmk)).
4.  All the following *Cardano wallet* 24-word phrases can then be copied & pasted into the "Lace" screen where it says "Import wallet".  If you cannot paste the whole phrase, make sure all individually selected words are an exact match.  

Likewise, list here any **exact web locations** from which your inheritors will need to construct:
- wallet holdings for all the blockchains on which you are listing resources below;
...
Here you must imagine a person trying to reconstruct your entire fortune from a cold start.  There cannot be a complete checklist for all of these, but it should include the following as applicable:  
...
- for each non-wallet resource:
  - copy all key pairs necessary to reconstruct that resource:  
    ...
    - (likewise any process for claiming staking rewards for these other address should be detailed)  
...
- list any other means of authenticating your login or identity  
  ...
  - including **login details for the registered email address**: likely needed for 2FA & other ID challenges.
...
- how (including in what jurisdiction, with what services *including URLs*) cryptocurrency should be converted to fiat
...
Therefore, unless you can think of other restraining factors that will prevent a single person from taking everything that you have — including the possibility of your executor being robbed by an antagonist who can combine the testament with its password — you can proceed with the placement when you have found **two people or agencies** that meet all the following criteria:
...
- each of these has had assurances of the other's trustworthiness while you are in sound mind and body;
- both of them together will not be inclined to secretly conspire against you.

If you cannot find two such people, or have no such faith that such trustworthiness is possible or can ever really be relied upon, your best choice is to keep any prepared document for your own reference and skip to the final section (LINK: ALTERNATIVES).

### 1. Choose the decryption password {#password-choice} 

Unless you plan to distribute this document to your inheritor *both **in** and **with*** the Frankenwallet — i.e. including them as a single package, perhaps walking through accessing your instructions ahead of time, at least by opening the file where you've prepared it inside the Frankenwallet — your own master copy of this document will reside on your host machine, encrypted with a separate password than the Frankenwallet.

{: .highlight }
> This should be a different password to avoid the possibility that an antagonist gaining access to the testament password will not also be able to raid your resources by obtaining & operating your Frankenwallet.
...
As anyone following the Frankenwallet instructions would agree, most people and their desktop computers will never maintain the security assurances necessary to work with cryptocurrency keys or passphrases.  You must assume general household computers will be infected with crypto-raiding malware and that your legacy would be lost the moment you opened your testament file on such a grossly insecure machine.

For that reason, you might want to advise them in a note, or whatever covering email encloses your attached testament file, as follows:
...
2.  Then, be prepared to execute those instructions securely: e.g. advising to buy a new computer, or obtain an old one, and install a fresh copy of Linux on it... or whatever you consider adequate security precautions (perhaps with technical assistance from your (IDENTIFIED HELPERS)).  
...
➤ Don't use a **rewritable CD**{: .text-red-300 }: these are weaker in both and logical structure.

It is absolutely vital that any media you prepare be **marked with the date on the media itself** rather than its containing envelope (another failing of memory sticks vs. optical discs).
...
➤ This is a chance to show a personal touch — ensuring the executor & perhaps other inheritors will see its indisputable and final significance — and to include brief instructions and/or completely reliable long-term contact information for the executor (i.e. the file-bearer).  
...
Given how crypto portfolio assets shift around, you will want to update the testament file in your Frankenwallet whenever you create or close a substantial wallet, account or substantial DeFi position: with no logistical difficulty, since you would be using the Frankenwallet for all these things anyway.

{: .new-title }
> tip
>
> You could also format your own personal file records of wallets & other resources so the key components can simply be copy & pasted into the separate testament document.

Then a newly burned CD can be **marked with the date** and sent out to your executor to replace all older copies: updating the "cover" instructions whenever necessary.  The rest of the process is in the hands of a "higher power" as they say. 😇  

## Alternatives (potentially) {#alternatives}

At the time of this writing, and for the five years of the author's experience preceding it, there is no generally trusted means of simulating the process of batching funds and information for delivery upon the holder's confirmed death or incapacitation.  In fact, even proving technically that someone has deceased (as with a [blockchain oracle](https://en.wikipedia.org/wiki/Blockchain_oracle) that a smart contract could use) is an infamously error-prone process.

### Use a commercial service (currently rare) {#outsourcing}

Now that you know what roles they would be fulfilling, you may feel confident evaluating whether any new company entering the crypto marketplace — on any blockchain — would be capable of doing this with enough technical and ethical reputation to reassure you whenever considering your own mortality.
...
- As long as you are living, you keep sending that message within that interval: if not, the "canary" assumes you are deceased or incapacitated and transfers the funds from your own accounts to the beneficiary.
